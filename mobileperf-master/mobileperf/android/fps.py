...
class SurfaceStatsCollector(object):
    ...
    def __init__(self, device, frequency, package_name, fps_queue, jank_threshold, use_legacy=False):
        ...
        self.jank_threshold = jank_threshold / 1000.0  # 转换为秒
        ...
        # 添加数据平滑处理
        self.frame_buffer = []  # 帧时间缓冲区
        self.buffer_size = 120  # 缓冲区大小(2秒@60fps)
        self.fps_history = []   # FPS历史记录
    ...
    def _calculate_results_new(self, refresh_period, timestamps):
        ...
        fps = min(60, int(round(1.0 / avg_interval)))
        ...
        for interval in frame_intervals:
            if prev_interval is not None:
                # 判断卡顿的两个条件：
                # 1. 当前帧间隔超过平均值的2倍
                # 2. 当前帧间隔超过33.3ms(30fps)
                if interval > max(2 * avg_interval, 0.0333):
                    jank_count += 1
            prev_interval = interval

        # 应用平滑处理
        self.fps_history.append(fps)
        if len(self.fps_history) > self.smooth_window:
            self.fps_history.pop(0)
        
        smoothed_fps = int(sum(self.fps_history) / len(self.fps_history))
        
        return smoothed_fps, jank_count
    ...
    def _calculator_thread(self, start_time):
        ...
        try:
            with open(fps_file, 'a+') as df:
                ...
                if self.fps_queue:
                    self.fps_queue.put({'fps_file': fps_file})
        except Exception as e:
            logger.exception(e)

        while True:
            try:
                data = self.data_queue.get(timeout=1)  # 添加超时
                ...
                if timestamps:
                    ...
                    logger.debug(f'FPS:{fps:2d} Jank:{jank}')
                    ...
                    try:
                        ...
                        fps_data = {
                            ...
                            "activity_window": self.focus_window,
                            "fps": fps,
                            "jank": jank,
...
class FPSMonitor(Monitor):
    ...
    def __init__(self, device_id, package_name=None, frequency=1.0, timeout=24 * 60 * 60, fps_queue=None,
                 jank_threshold=166, use_legacy=False):
        '''构造器
...
if __name__ == '__main__':
    ...
    monitor = FPSMonitor('S30PQkRa0500702', "com.yangcong345.cloud.ui.launcher", 1)
    #  mate 9 android8.0
