                    ...
                    if not self.mem_queue:#为了本地单个文件运行
                        ...
                        try:
                            ...
                            for package_pid_pss in mem_device_snapshot.package_pid_pss_list:
                                print(f"meminfo_______________________________________________: {package_pid_pss}")
                                # Creating a dictionary for meminfo_data
                                meminfo_data = {
                                    "device_id": self.device.get_device_id(),
                                    "datetime": datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                    "totalmem": mem_device_snapshot.totalmem,
                                    "freemem": mem_device_snapshot.freemem,
                                    "package_name": package_pid_pss['package'],
                                    "pid": package_pid_pss['pid'],
                                    "pss": package_pid_pss['pss'],
